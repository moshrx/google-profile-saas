  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      ...
      const templateParams = {
        to_email: "peiwebstudio@gmail.com",
        from_email: email,
...
Matched Grants: ${matched.map(g => g.name).join(", ") || "No specific grants matched"}`,
      ...
      const leads = JSON.parse(localStorage.getItem("listedpei_grant_leads") || "[]");
      leads.push({
        email,
        answers,
        answersLabel,
          ...
          <div className="text-center mb-8 sm:mb-12">
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-slate-900 mb-3 sm:mb-4 tracking-tight">
              ...
              <span className="text-gradient">
                {matchedGrants.length} grant{matchedGrants.length !== 1 ? "s" : ""}
              </span>
            </h1>
          ...
          <div className="space-y-4 sm:space-y-6 mb-8 sm:mb-12">
            {matchedGrants.length > 0 ? (
              matchedGrants.map((grant, idx) => (
                <div
                  key={grant.id}
                  className="card p-4 sm:p-6 md:p-8 border-l-4 border-l-primary-500 animate-fade-in-up"
                  style={{ animationDelay: `${idx * 0.1}s` }}
                >
                  <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6">
                    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-primary-50 rounded-xl sm:rounded-2xl flex items-center justify-center text-2xl sm:text-3xl flex-shrink-0">
                      {grant.icon}
                    </div>
                    <div className="flex-1">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                        <h3 className="text-lg sm:text-xl font-bold text-slate-900">{grant.name}</h3>
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-primary-100 text-primary-700 w-fit">
                          {grant.maxAmount}
                        </span>
                      </div>
              ...
              <div className="card p-8 sm:p-12 text-center">
                ...
                <p className="text-slate-500 text-base sm:text-lg mb-6 max-w-md mx-auto">
                  Based on your answers, we couldn't find specific grants that match all your criteria. 
                  However, Innovation PEI has many programs available!
                </p>
                <a
                  href="https://www.princeedwardisland.ca/en/information/innovation-pei"
                  target="_blank"
          ...
          <div className="card bg-slate-900 text-white p-6 sm:p-8 md:p-10 relative overflow-hidden">
            ...
            <div className="relative z-10 text-center">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-black mb-3 sm:mb-4">
                Ready to apply for your grants?
              </h3>
              <p className="text-slate-400 text-base sm:text-lg mb-6 sm:mb-8 max-w-xl mx-auto">
                Book a free call with our team to discuss your eligibility and get help with your application.
              </p>
              <a
                href="mailto:peiwebstudio@gmail.com?subject=Grant%20Consultation%20-%20GrantReady%20PEI"
                className="inline-flex items-center justify-center gap-2 bg-primary-500 hover:bg-primary-600 text-white font-bold px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl transition-all hover:-translate-y-1 shadow-xl shadow-primary-500/20 text-base sm:text-lg"
              >
                Book a free call
      ...
      <div className="min-h-screen bg-[#FAFBFF] flex items-start sm:items-center justify-center p-4 sm:p-6 pt-24 sm:pt-6">
        ...
        <div className="w-full max-w-xl animate-fade-in">
          ...
          <div className="card p-6 sm:p-8 md:p-10 text-center">
            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-primary-50 rounded-2xl sm:rounded-3xl flex items-center justify-center text-3xl sm:text-4xl mb-4 sm:mb-6 mx-auto">
              📧
            </div>
            ...
            <p className="text-slate-500 text-sm sm:text-base mb-6 sm:mb-8 max-w-sm mx-auto">
              We'll send you a personalized summary of the Innovation PEI grants you qualify for, plus helpful tips for your application.
            </p>
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-4">
              ...
              <button
                ...
                className="btn-primary w-full bg-primary-600 hover:bg-primary-700 disabled:opacity-50"
              >
                {isSubmitting ? (
                  <>
            ...
            <button
              ...
              className="mt-4 text-slate-400 text-sm hover:text-slate-600 transition-colors"
            >
      ...
      <div className="w-full max-w-2xl animate-fade-in">
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-slate-500 text-base sm:text-lg">
            Answer 5 quick questions to discover Innovation PEI grants you qualify for
          </p>
        </div>
        ...
        <div className="card p-4 sm:p-6 md:p-8 lg:p-10">
          <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 pb-4 sm:pb-6 border-b border-slate-50">
            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-slate-50 rounded-xl sm:rounded-2xl flex items-center justify-center text-2xl sm:text-3xl shadow-inner-light flex-shrink-0">
              {currentStepData.icon}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] sm:text-xs font-black text-primary-600 uppercase tracking-[0.15em] sm:tracking-[0.2em] mb-1">
            ...
            {currentStepData.options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`relative p-4 sm:p-5 rounded-xl sm:rounded-2xl border-2 text-left transition-all duration-300 group ${
                    isSelected
        ...
        <p className="text-center text-xs text-slate-400 mt-4 sm:mt-6 px-4">
          Takes less than 2 minutes • Your information is never shared
        </p>
      </div>
    </div>
  );
