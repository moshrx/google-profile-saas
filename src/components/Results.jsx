...
function CopyCard({ label, content, badge }) {
  ...
  return (
    <div className="card-result p-5 border border-slate-100 mb-4 group relative overflow-hidden transition-all duration-300 hover:border-primary-200">
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-1">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{label}</span>
          {badge && (
            <span className="px-2 py-0.5 bg-primary-50 text-primary-700 text-[10px] font-bold rounded-full uppercase tracking-tighter ring-1 ring-primary-100 ml-1">
              {badge}
            </span>
          )}
        </div>
...
export default function Results({ result, formData, onReset, onExportPDF }) {
  ...
  if (!result || !formData) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 text-center">
        <div className="card max-w-md p-8 shadow-xl">
  ...
  // Handle Send Kit Email via Web3Forms (Feature 1)
...
PHOTO TIPS: ${(result.photoTips || []).join("\n")}`,
  ...
  // Handle Website Lead via Web3Forms (Feature 4)
  ...
  return (
    <div className="min-h-screen bg-[#FAFBFF] pb-24 pt-12 px-6 overflow-hidden">
      <ToastContainer toasts={toasts} />
      
      {/* ── Background Flair ─────────────────────────────────────────── */}
      <div className="fixed inset-0 pointer-events-none opacity-50">
        <div className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-primary-100/30 blur-[120px] rounded-full"></div>
        <div className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-accent-100/20 blur-[120px] rounded-full"></div>
      </div>
      ...
      <div className="max-w-4xl mx-auto relative animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12">
          <div>
            <div className="flex items-center gap-2 mb-4">
              <span className="bg-primary-500 text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg shadow-primary-500/20">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            <button 
              onClick={onReset} 
              className="btn-secondary h-14 px-6 flex-1 md:flex-none border-slate-200"
            >
              Start New
            </button>
            <button
              onClick={() => onExportPDF(result, formData)}
              className="btn-primary h-14 px-8 flex-1 md:flex-none"
            >
        ...
        <div className="card shadow-md p-6 mb-8 border-2 border-primary-100 bg-gradient-to-r from-white to-primary-50">
          <h2 className="text-xl font-bold text-slate-800 mb-2">Get your full kit delivered to your inbox</h2>
          <form onSubmit={handleSendKitEmail} className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              className="btn-primary bg-slate-800 hover:bg-slate-900 px-8 disabled:opacity-50"
            >
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Descriptions</span>
            <div className="h-px flex-1 bg-slate-200" />
          </div>
          ...
          <div className="flex items-center gap-3 mb-3 mt-8">
            <div className="h-px flex-1 bg-slate-200" />
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Google Posts (5)</span>
            <div className="h-px flex-1 bg-slate-200" />
          </div>
          ...
          <div className="flex items-center gap-3 mb-3 mt-8">
            <div className="h-px flex-1 bg-slate-200" />
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Review Responses</span>
            <div className="h-px flex-1 bg-slate-200" />
          </div>
          ...
          <div className="flex items-center gap-3 mb-3 mt-8">
            <div className="h-px flex-1 bg-slate-200" />
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">📸 Photo Tips</span>
            <div className="h-px flex-1 bg-slate-200" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {(result.photoTips || []).map((tip, i) => (
              <div key={i} className="card p-4 border border-slate-100 hover:border-primary-100 transition-all flex flex-col justify-between">
                <div>
                ...
                <button
                  ...
                  className="mt-4 text-[10px] font-bold text-primary-600 uppercase tracking-wider self-start hover:text-primary-700"
                >
                  Copy Tip
                </button>
              </div>
          ...
          <div className="flex items-center gap-3 mb-3 mt-8">
            <div className="h-px flex-1 bg-slate-200" />
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">🔍 Keywords to Target</span>
            <div className="h-px flex-1 bg-slate-200" />
          </div>
          <div className="card shadow-md p-6 border border-slate-100">
            <div className="space-y-6">
              {[
                ...
                <div key={group.label}>
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">{group.label}</h3>
                  <div className="flex flex-wrap gap-2">
                    {group.list.map((kw) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all hover:scale-105 active:scale-95 ${group.color}`}
                      >
                        {kw}
                      </button>
                    ))}
            ...
            <button
              ...
              className="mt-8 w-full btn-secondary text-primary-700 border-primary-100 hover:bg-primary-50"
            >
              Copy All Keywords
            </button>
          </div>
          ...
          <div className="flex items-center gap-3 mb-3 mt-8">
            <div className="h-px flex-1 bg-slate-200" />
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Common FAQs</span>
            <div className="h-px flex-1 bg-slate-200" />
          </div>
          ...
          {(result.faqs || []).map((faq, i) => (
            <div key={i} className="space-y-2">
              <div className="flex items-center gap-2 mb-1 pl-1">
                <span className="text-xs font-bold text-primary-600 bg-primary-50 px-2 py-0.5 rounded uppercase font-outfit tracking-wider">
                  Q&A {i + 1}
                </span>
              </div>
          ...
          <div className="flex items-center gap-3 mb-3 mt-8">
            <div className="h-px flex-1 bg-slate-200" />
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Suggested Categories</span>
            <div className="h-px flex-1 bg-slate-200" />
          </div>

          <div className="card-result p-6 border border-slate-100 bg-slate-50/50">
            <div className="space-y-3">
              {result.categories.map((cat, i) => (
                <div key={i} className="flex items-center gap-4">
                  ...
                  <span className={`font-semibold ${i === 0 ? "text-slate-800" : "text-slate-500"}`}>{cat}</span>
                  {i === 0 && <span className="text-[10px] font-bold text-primary-600 uppercase tracking-widest ml-auto">Primary</span>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-20 bg-slate-900 rounded-[2.5rem] p-6 md:p-16 shadow-2xl text-white overflow-hidden relative group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 relative z-10">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-white text-[10px] font-black uppercase tracking-widest mb-8 border border-white/10">
            ...
            <div className="glass-dark border-white/5 p-6 md:p-10 rounded-[2rem]">
              {websiteLeadSent ? (
                <div className="h-full flex flex-col items-center justify-center text-center py-12">
                  ...
                  <p className="text-slate-400">We'll reach out to discuss your mockup soon.</p>
                </div>
              ) : (
                <>
                  <h3 className="text-2xl font-black mb-8">Get a Free Mockup</h3>
                  <form onSubmit={handleWebsiteLeadSubmit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full px-6 py-4 rounded-2xl bg-white/5 border border-white/10 text-white placeholder:text-slate-600 focus:bg-white/10 focus:border-primary-500/50 outline-none transition-all"
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        className="w-full px-6 py-4 rounded-2xl bg-white/5 border border-white/10 text-white placeholder:text-slate-600 focus:bg-white/10 focus:border-primary-500/50 outline-none transition-all resize-none"
                    ...
                    <button
                      ...
                      className="w-full py-5 bg-primary-500 hover:bg-primary-600 text-white rounded-2xl font-black text-lg transition-all hover:shadow-[0_0_30px_rgba(14,165,233,0.3)] disabled:opacity-50 mt-4"
                    >
